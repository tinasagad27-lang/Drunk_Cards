...
<body>
    <div class="container mt-5 text-center fade-in">
        ...
        <?php if (!empty($cards)): ?>
            ...
            <h4 class="text-warning">Category: <?= htmlspecialchars($cards[0]['category']); ?></h4>

            <!-- Display Question -->
            <div class="card-container mx-auto mt-3">
                <div class="card">
                    <h3><?= htmlspecialchars($cards[0]['question']); ?></h3>
                </div>
            </div>
            ...
            <div class="btn-container">
                <form action="<?= site_url('shuffle_game_controller/next'); ?>" method="POST">
                    <input type="hidden" name="category" value="<?= htmlspecialchars($cards[0]['category']); ?>">
